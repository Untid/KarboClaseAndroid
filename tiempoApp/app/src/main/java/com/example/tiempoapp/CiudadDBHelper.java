...
public class CiudadDBHelper extends SQLiteOpenHelper {
    ...
    public void insertarCiudad(Ubicacion ubicacion) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("nombre", ubicacion.getNombre());
    ...
    public List<Ubicacion> obtenerTodasLasCiudades() {
        List<Ubicacion> ciudades = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM ciudades", null);
        if (cursor.moveToFirst()) {
            do {
                ciudades.add(new Ubicacion(
                        cursor.getString(1),
                        cursor.getDouble(2),
                        cursor.getDouble(3)
                ));
